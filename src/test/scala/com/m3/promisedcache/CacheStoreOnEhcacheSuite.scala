...
class CacheStoreOnEhcacheSuite extends FunSuite with ShouldMatchers {

  val cacheName = "CacheStoreOnEhcacheSuite_test_" + System.currentTimeMillis()
  val ehcache = new Cache(cacheName, 5000, false, false, 60, 60)
  CacheManager.getInstance().addCache(ehcache)
  implicit val cacheStore: CacheStore = new CacheStoreOnEhcache(ehcache)
